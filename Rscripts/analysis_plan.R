...
analysis_plan = drake_plan(
  ...
  ana.data.1.1 <- ana.data %>% 
    filter(Action == "use" | Action == "share",does.R.primary ==1, !is.na(Values)) %>% 
    mutate(Year = 2020 - Year),
  
  
  eng.clm.g <- clm(Values ~ University * Year + Gender, data = ana.data.1.1),
  ...
  eng.clmm.f <- clmm(Values ~ University * Year + (1|NO), data = ana.data.1.1),
  
  summary(eng.clmm.f),
  
  eng.clmm2.f1 <- clmm2(Values ~ University * Year, random = NO,  data = ana.data.1.1, Hess = TRUE),
  eng.clmm2.f2 <- clmm2(Values ~ University * Year, random = NO, nominal = ~University : Year, data = ana.data.1.1, Hess = TRUE),
  eng.clmm2.f3 <- clmm2(Values ~ University * Year, random = NO, nominal = ~University, data = ana.data.1.1, Hess = TRUE),
  eng.clmm2.f4 <- clmm2(Values ~ University * Year, random = NO, nominal = ~Year, data = ana.data.1.1, Hess = TRUE),
  
  anova(eng.clmm2.f1, eng.clmm2.f2),
  anova(eng.clmm2.f1, eng.clmm2.f3),
  anova(eng.clmm2.f1, eng.clmm2.f4),
  
  #Non-convergent models using clmm2. Indications of violation of proportional odds assumption 
  #Suggestion: Keep eng.clmm.f as final model and be clear about violation of proportional odds assumption. The practical implication of this is that we interpret response along the continuum of likert categories but refrain from discussing single categories.
  ...
  ana.data.1.2a <- bndata %>% 
    ...
    filter(Gender!= "Other", T_R == "1_1", !is.na(Values)),
  ...
  imp.clmm2.a.f1 <- clmm2(Values ~ Domain + Gender, random = NO,  data = ana.data.1.2a),
  imp.clmm2.a.f2 <- clmm2(Values ~ Domain + Gender, random = NO, nominal = ~Domain, data = ana.data.1.2a),
  imp.clmm2.a.f3 <- clmm2(Values ~ Domain + Gender, random = NO, nominal = ~Gender, data = ana.data.1.2a)  ,
  ...
  imp.clmm.b.f <- clmm(Values ~ Domain + Gender + (1|NO), data = ana.data.1.2b)  ,
  
  summary(imp.clmm.b.f),
  
  #imp.clmm2.b.f1 <- clmm2(Values ~ Domain + Gender, random = NO,  data = ana.data.1.2b)
  #imp.clmm2.b.f2 <- clmm2(Values ~ Domain + Gender, random = NO, nominal = ~Domain, data = ana.data.1.2b)
  #imp.clmm2.b.f3 <- clmm2(Values ~ Domain + Gender, random = NO, nominal = ~Gender, data = ana.data.1.2b)  
  
  #non-successful convergence using clmm2. Suggestion: Keep imp.clmm.b.f as final model and be clear about violation of proportional odds assumption. The practical implication of this is that we interpret response along the continuum of likert categories but refrain from discussing single categories.
  ...
  imp.clmm2.c.f1 <- clmm2(Values ~ Domain, random = NO,  data = ana.data.1.2c),
  imp.clmm2.c.f2 <- clmm2(Values ~ Domain, random = NO, nominal = ~Domain, data = ana.data.1.2c),
  
  anova(imp.clmm2.c.f1, imp.clmm2.c.f2),
  ...
  imp.clmm2.d.f1 <- clmm2(Values ~ Domain, random = NO,  data = ana.data.1.2d),
  imp.clmm2.d.f2 <- clmm2(Values ~ Domain, random = NO, nominal = ~Domain, data = ana.data.1.2d),
  
  anova(imp.clmm2.d.f1, imp.clmm2.d.f2),
  ...
  use.clm.g <- clm(Values ~  Action + Aspect + Gender, data = ana.data.1.3),
  ...
  use.clmm.nom2 <- clmm2(Values ~ Action + Gender, nominal = ~Gender, random = NO, data = ana.data.1.3),
  use.clmm.nom3 <- clmm2(Values ~ Action + Gender, nominal = ~Action, random = NO, data = ana.data.1.3),
  ...
  #set Gender as nominal to relax proportional odds assumption
  use.clmm2.f <- clmm2(Values ~ Action + Gender, nominal = ~Gender, random = NO, data = ana.data.1.3, Hess = TRUE),
